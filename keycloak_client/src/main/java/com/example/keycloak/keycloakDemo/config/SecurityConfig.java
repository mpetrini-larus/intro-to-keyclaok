...
//              configures the access restriction policies
                .authorizeHttpRequests(authRequest -> authRequest
                        .requestMatchers("/api/user/roles").hasAnyAuthority("admin")
    ...
    Converter<Jwt, AbstractAuthenticationToken> authenticationConverter() {
        ...
        jwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(source -> {
            Collection<String> roles = Collection.class.cast(
                    Map.class.cast(source.getClaims().getOrDefault("realm_access", Map.of()))
                            .getOrDefault("roles", Collections.EMPTY_LIST)
            );
